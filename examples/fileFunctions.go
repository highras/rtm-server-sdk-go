...
func demoSendFiles(client *rtm.RTMServerClient, filePath string) {
	...
	//-- sync send P2P file
	mtime, err := client.SendFile(fromUid, toUid, data, filename, "", nil)
	locker.print(func() {
		if err == nil {
	...
	//-- async send P2P file
	_, err = client.SendFile(fromUid, toUid, data, filename, "", nil, func(mtime int64, errorCode int, errInfo string) {
		locker.print(func() {
	...
	mtime, err = client.SendFiles(fromUid, toUids, data, filename, "", nil)
	locker.print(func() {
		if err == nil {
	...
	_, err = client.SendFiles(fromUid, toUids, data, filename, "", nil, func(mtime int64, errorCode int, errInfo string) {
		locker.print(func() {
			if errorCode == fpnn.FPNN_EC_OK {
	...
	mtime, err = client.SendGroupFile(fromUid, groupId, data, filename, "", nil)
	locker.print(func() {
		if err == nil {
	...
	_, err = client.SendGroupFile(fromUid, groupId, data, filename, "", nil, func(mtime int64, errorCode int, errInfo string) {
		locker.print(func() {
			if errorCode == fpnn.FPNN_EC_OK {
	...
	//-- sync send room file
	mtime, err = client.SendRoomFile(fromUid, roomId, data, filename, "", nil)
	locker.print(func() {
		if err == nil {
	...
	_, err = client.SendRoomFile(fromUid, roomId, data, filename, "", nil, func(mtime int64, errorCode int, errInfo string) {
		locker.print(func() {
			if errorCode == fpnn.FPNN_EC_OK {
	...
	mtime, err = client.SendBroadcastFile(adminUid, data, filename, "", nil)
	locker.print(func() {
		if err == nil {
	...
	_, err = client.SendBroadcastFile(adminUid, data, filename, "", nil, func(mtime int64, errorCode int, errInfo string) {
		locker.print(func() {
			if errorCode == fpnn.FPNN_EC_OK {
