...
func deleteChat(client *rtm.RTMServerClient) {
	...
	//-- sync mode
	err := client.DelChat(mid, fromUid, toUid, rtm.MessageType_P2P)
	locker.print(func(){
			if err == nil {
				fmt.Printf("DelChat in sync mode is fine.\n")
			} else {
				fmt.Printf("DelChat in sync mode error, err: %v\n", err)
			}
		})

	//-- async mode
	err = client.DelChat(mid, fromUid, groupId, rtm.MessageType_Group, func(errorCode int, errInfo string){
		locker.print(func(){
				if errorCode == fpnn.FPNN_EC_OK {
						fmt.Printf("DelChat in async mode is fine.\n")
					} else {
						fmt.Printf("DelChat in async mode error, error code: %d, error info:%s\n", errorCode, errInfo)
					}
			})
		})
	...
	if err != nil {
		locker.print(func(){
				fmt.Printf("DelChat in async mode error, err: %v\n", err)
			})
	}
}
